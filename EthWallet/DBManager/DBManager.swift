...
//    }
    ...
    func createTable(keys:Array<String>) -> Bool {
        ...
        databaseQueue?.inDatabase({ (db) in
            ...
            if
                ...
                let rs = try? db.executeUpdate(sql, withArgumentsIn: []),
                rs == true{
                result = true
    ...
    func examine() -> Bool {
        ...
        databaseQueue?.inDatabase({ (db) in
            if
                let rs = try? db.tableExists(self.tableName),
                rs == true{
                result = true
            }else{
                result = false
    ...
    func insertData(data:Dictionary<String,Any>) -> Bool {
        ...
        databaseQueue?.inDatabase({ (db) in
            if
                self.validateSql(db: db, sql: sql) == true,
                let rs = try? db.executeUpdate(sql, withArgumentsIn: values),
                rs == true{
                result = true
    ...
    func insertDatas(datas:Array<Dictionary<String,Any>>) -> Bool {
        ...
        databaseQueue?.inTransaction({ (db, rollBack) in
            ...
            datas.forEach({ (dict) in
                num += 1
                if
                    let rs = try? db.executeUpdate(sql, withArgumentsIn: Array(dict.values)),
                    rs == true{
                    result = true
    ...
    func updateData(keyValues:Dictionary<String,Any> = Dictionary<String,Any>(),condition:Array<String> = []) -> Bool {
        ...
        databaseQueue?.inDatabase({ (db) in
            ...
            //var Sql = String.init(sql.suffix(sql.count - 1))
