...
class ExportPKCodeController: BaseViewController {
    ...
    @IBOutlet weak var confirmView: UIView!{
        didSet{
            ...
            confirmView.layer.cornerRadius = 2
            confirmView.layer.shadowOpacity = 1
            confirmView.layer.shadowRadius = 10
            confirmView.layer.shadowColor = JX10101aShadowColor.cgColor
            confirmView.layer.shadowOffset = CGSize(width: 0, height: -3)
            //let path = CGPath(rect: CGRect(x: -24, y: -40, width: kScreenWidth, height: 184), transform: nil)
            //confirmView.layer.shadowPath = path
        }
    }
    ...
    @IBOutlet weak var showButton: JXShadowButton!
    ...
    @IBAction func show(_ sender: Any) {
        self.codeImageView.image = self.code(self.privateKeyStr)
